...
public class PassthroughDrawPointer : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        // NEW: Show green when just hovering over the marker (before trigger)
        if (!hasStartedDrawingFromMarker && markerHit && !isTriggerHeld)
        {
            if (markerRenderer != null && markerRenderer.material != greenArrow)
                markerRenderer.material = greenArrow;
            debugText.text = "You Can Draw from here!";
            debugText.color = Color.green;
        }

        // Start drawing if trigger pressed on marker
        if (!hasStartedDrawingFromMarker && markerHit && isTriggerHeld)
        {
        ...
        // Continue drawing anywhere after start
    ...
    {
        if (clip != null && clip != lastPlayedClip)
        {
            audioSource.Stop();
