...
public class PassthroughDrawPointer : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        if (hasHit)
        {
            ...
            if (startPointMarkerInstance != null)
            {
                ...
                bool markerHit = distToMarker < 0.1f;

                if (startPointMarkerInstance.TryGetComponent(out MarkerController colorCtrl))
                {
                    colorCtrl.SetSelected(markerHit);
                }
            }
        ...
        else
        {
            ...
            // Reset marker color if ray misses everything
            if (startPointMarkerInstance != null &&
                startPointMarkerInstance.TryGetComponent(out MarkerController colorCtrl))
            {
                colorCtrl.SetSelected(false);
            }
        }
    }
    ...
    private void UpdateStartMarkerPosition()
    {
        ...
        if (room.Raycast(ray, rayDistance, LabelFilter.FromEnum(MRUKAnchor.SceneLabels.FLOOR), out RaycastHit hit, out MRUKAnchor anchor))
        {
            ...
            startPointMarkerInstance.transform.position = Vector3.Lerp(
                startPointMarkerInstance.transform.position,
                markerPosition,
                Time.deltaTime * 5f
            );

            // Smooth rotation update
            startPointMarkerInstance.transform.rotation = Quaternion.Slerp(
                startPointMarkerInstance.transform.rotation,
                targetRotation,
                Time.deltaTime * 5f
            );
